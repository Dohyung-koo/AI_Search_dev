...
public class ArDbToDbManager {
	...
	public void Start() {
		//System.out.println("AR데이터 수집시작");
		openDB();
		db2DbStart();
		closeDB();
		//System.out.println("AR데이터 수집종료");
	}
	/**
	 * DB2DB DB연결
	 */
	public void openDB() {
		...
		System.out.println("OPEN ORACLE DB [" + DB_INFO.AR_ORACLE_URL + "]");
		String oraUrl = DB_INFO.AR_ORACLE_URL;
		String oraUser = DB_INFO.AR_ORACLE_USER;
		String oraPassword = DB_INFO.AR_ORACLE_PASSWORD;
		String oraDriver = DB_INFO.AR_ORACLE_DRIVER;
...
//			System.out.println(str);
	...
	/**
	 * DB2DB Oracle 테이블 1개 전송 VW_CONTENTS_INFO
	 */
...
//		System.out.println("start oracle -> mysql");
		for (int i = 0; i < DB2DB.AR_ORACLE_TABLE.length; i++) {
			DBConnector.truncateTable(myConn, DB2DB.AR_MYSQL_TABLE[i]);
			
			HashMap<String, String> tableDataMap = new HashMap<String, String>();

			String tableName = DB2DB.AR_ORACLE_TABLE[i];
			String query = DB2DB.all_Select(tableName);
//			System.out.println("ARENT.VW_CONTENTS_INFO : "+query);
			try {
			...
			// MySQl에 Data Insert
			insertSQL(myConn, DB2DB.AR_MYSQL_TABLE[i], mapList, rs, pstm,null);
			mapList.clear();
		}
		...
		try {
			...
			DB2DB.generateAllUpdateSql(buf, DB2DB.AR_LOOKUP_TABLE, new String[] { "SYNC_STATUS" });
			pstm = oracleConn.prepareStatement(buf.toString());
			pstm.setString(1, "1");
			pstm.executeUpdate();
			System.out.println("UPDATE SUCCESS "+DB2DB.AR_LOOKUP_TABLE+" TABLE");
		} catch (SQLException e) {
			System.out.println(e.getMessage());
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	private int tableDataCount(Connection myConn, PreparedStatement pstm, ResultSet rs)
			throws SQLException {
		int a = 0;
		for (int i = 0; i < DB2DB.AR_MYSQL_TABLE.length; i++) {
			String query = DB2DB.all_Select(DB2DB.AR_MYSQL_TABLE[i]);
