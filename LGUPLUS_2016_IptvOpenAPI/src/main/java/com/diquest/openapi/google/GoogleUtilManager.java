...
public enum GoogleUtilManager {
	INSTANCE;
	
	private GoogleUtilManager() {
...
//		static String tvgPopAuthKey = "lguplusufsearch";
	 ...
	 * @return vo
	 */
	public static GoogleRequest checkGoogleInfo(GoogleRequest vo, String decodeYn) {

		if(vo.getStructuredQuery()== null){
			vo.setErrorCode(ErrorString.GENERAL.OPERATOR_INTERNAL_ERROR	);
			return vo;
		}
		String queryIntent = vo.getStructuredQuery().getQueryIntent();									// (필수) 수집 영역
		
		if ( StringUtils.isEmpty(queryIntent)) {
			//queryIntent가 없을경우
			vo.setErrorCode(ErrorString.GENERAL.OPERATOR_INTERNAL_ERROR	);
			return vo;
		} else if(!("SWITCH_CHANNEL".equals(queryIntent) ||"ENTITY_SEARCH".equals(queryIntent) || "PLAY_TVM".equals(queryIntent) || "PLAY".equals(queryIntent)
				|| "SEARCH".equals(queryIntent))) {
			vo.setErrorCode(ErrorString.GENERAL.OPERATOR_INTERNAL_ERROR	);
			return vo;

		} else if("ENTITY_SEARCH".equals(queryIntent) || "PLAY_TVM".equals(queryIntent)) {
			if(vo.getStructuredQuery().getEntitiesList()==null&&StringUtils.isEmpty(vo.getStructuredQuery().getSearchQuery())){
				vo.setErrorCode(ErrorString.GENERAL.OPERATOR_INTERNAL_ERROR	);
				return vo;
			}
			Entities e =vo.getStructuredQuery().getEntitiesList().get(0);
			if(StringUtils.isEmpty(e.getContentId())&&StringUtils.isEmpty(e.getTitle())
				&&StringUtils.isEmpty(vo.getStructuredQuery().getSearchQuery())){
				vo.setErrorCode(ErrorString.GENERAL.OPERATOR_INTERNAL_ERROR	);
				return vo;
			}
		}  else if("PLAY".equals(queryIntent) || "SEARCH".equals(queryIntent)) {
			if(StringUtils.isEmpty(vo.getStructuredQuery().getSearchQuery())){
				//검색어가 없을때
				vo.setErrorCode(ErrorString.GENERAL.OPERATOR_INTERNAL_ERROR	);
				return vo;
			}
		} else if("SWITCH_CHANNEL".equals(queryIntent)) {
			if(StringUtils.isEmpty(vo.getStructuredQuery().getChannelName()) && StringUtils.isEmpty(vo.getStructuredQuery().getChannelNumber()
			)&& StringUtils.isEmpty(vo.getStructuredQuery().getChannelId())){
				vo.setErrorCode(ErrorString.SWITCH_CHANNEL.CHANNEL_NOT_AVAILABLE);
			}
		}
