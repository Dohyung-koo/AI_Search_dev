...
public class VideoService {
	...
	public VideoVO parseInput(String input) throws Exception {
		VideoVO searchVO = new VideoVO();
	 ...
	 * @return
	 */
	public QuerySet makeQuerySet(VideoVO searchVO) throws Exception {
		...
		// 통합검색 여부 체크
		if (w != null && w.equals("IPTV") && section != null && !section.equals("")) {
			sectionListArr = new String[] { section };
		} else if (w != null && w.equals("IPTV_QUICK") && (section == null || section.equals(""))) {
	 ...
	 * @return
	 */
	public QuerySet makeNewQuerySet(VideoVO searchVO) throws Exception {
		String w = searchVO.getW();
	 ...
	 * @return
	 */
	public OpenAPIVideoResponse search(VideoVO searchVO, int keyword_byte) throws Exception {
		OpenAPIVideoResponse resultResponse = new OpenAPIVideoResponse();
		...
		try {
			...
			if (rs >= -1) {
				...
				resultResponse = makeResponse(searchVO, queryList, resultList, resultResponse, searchVO.getSectionListArr());
				// System.out.println("### search success
				// VideoService.searchTotal");
				logger.debug("[SEARCH SUCCESS] search");
			} else {

				// System.out.println("### search fail VideoService.search() : "
				// + rs + " - " + command.getException());
				logger.debug("[SEARCH FAIL] search : " + rs + " - " + command.getException());
				resultResponse.setErrorResponse(UtilManager.makeRsInfo(rs));
			}
			...
			e.printStackTrace();
			resultResponse.setErrorResponse(UtilManager.makeError("1000"));
		}
	 ...
	 * @return
	 */
	public OpenAPIVideoResponseJson searchJson(VideoVO searchVO, int keyword_byte) throws Exception {
	 ...
	 * @return
	 */
	public OpenAPIVideoQuickResponse searchQuick(VideoVO searchVO, int keyword_byte) throws Exception {
		OpenAPIVideoQuickResponse resultResponse = new OpenAPIVideoQuickResponse();
		...
		try {
			...
			if (rs >= -1) {
				...
				resultResponse = makeQuickResponse(searchVO, queryList, resultList, resultResponse, searchVO.getSectionListArr());
				// System.out.println("### search success
				// VideoService.searchQuick");
				logger.debug("[SEARCH SUCCESS] searchQuick");
			} else {
				// System.out.println("### search fail VideoService.searchQuick
				// : " + rs + " - " + command.getException());
				logger.debug("[SEARCH FAIL] searchQuick : " + rs + " - " + command.getException());
				resultResponse.setErrorResponse(UtilManager.makeRsInfo(rs));
			}
		} catch (Exception e) {
			logger.error("[EXCEPTION START] " + e.toString());
			e.printStackTrace();
			resultResponse.setErrorResponse(UtilManager.makeError("2000"));
			// System.out.println("=+=+=+=+=+=+=+=+=+=+=+=+=+= EXCEPTION END
			// =+=+=+=+=+=+=+=+=+=+=+=+=+=");
		}
	 ...
	 * @return
	 */
	public OpenAPIVideoQuickResponseJson searchQuickJson(VideoVO searchVO, int keyword_byte) throws Exception {
	 ...
	 * @return
	 */
	private Query makeQuery(VideoVO searchVO) throws Exception {
		String w = searchVO.getW();
		String section = searchVO.getSection();
		...
		if (groupbySet != null && (!collectionName.equals("I30_AUTO_COMPLETE")) && (!collectionName.equals("I30_CHANNEL")))
...
//		 System.out.println("### Query : " + 	 queryParser.queryToString(query));
	 ...
	 * @return
	 */
	private Query makeNewQuery(VideoVO searchVO) throws Exception {
		String w = searchVO.getW();
		String section = searchVO.getSection();
		String collectionName = searchVO.getCollection();
		int pagenum = Integer.parseInt(searchVO.getPg()); // 페이지번호
...
//		 System.out.println("### Query : " + queryParser.queryToString(query));
	 ...
	 * @return
	 */
	private SelectSet[] makeSelectSet(VideoVO searchVO) throws Exception {
	 ...
	 * @return
	 */
	private WhereSet[] makeWhereSet(VideoVO searchVO) throws Exception {
...
//				whereList.add(new WhereSet(Protocol.WhereSet.OP_OR));
			...
			if (searchVO.getW().equals("IPTV")) {
				...
				//20170929 CAST_NAME 제외요청 반영
//				whereList.add(new WhereSet(Protocol.WhereSet.OP_OR));
//				whereList.add(new WhereSet("IDX_CAST_NAME_MAX_D", Protocol.WhereSet.OP_HASALL, searchVO.getQ()));
//				whereList.add(new WhereSet(Protocol.WhereSet.OP_OR));
//				whereList.add(new WhereSet("IDX_CAST_NAME_MAX_RW", Protocol.WhereSet.OP_HASALL, searchVO.getQ()));
//				whereList.add(new WhereSet(Protocol.WhereSet.OP_OR));
//				whereList.add(new WhereSet("IDX_CAST_NAME_MAX_TW", Protocol.WhereSet.OP_HASALL, searchVO.getQ()));
//				whereList.add(new WhereSet(Protocol.WhereSet.OP_OR));
//				whereList.add(new WhereSet("IDX_CAST_NAME_MAX_ENG_B", Protocol.WhereSet.OP_HASALL, searchVO.getQ()));
				...
				//20170929 CAST_NAME 제외요청 반영
//				whereList.add(new WhereSet(Protocol.WhereSet.OP_OR));
//				whereList.add(new WhereSet("IDX_CAST_NAME_ENG_B", Protocol.WhereSet.OP_HASALL, searchVO.getQ()));
//				whereList.add(new WhereSet(Protocol.WhereSet.OP_OR));
//				whereList.add(new WhereSet("IDX_CAST_NAME_D", Protocol.WhereSet.OP_HASALL, searchVO.getQ()));
//				whereList.add(new WhereSet(Protocol.WhereSet.OP_OR));
//				whereList.add(new WhereSet("IDX_CAST_NAME_RW", Protocol.WhereSet.OP_HASALL, searchVO.getQ()));
//				whereList.add(new WhereSet(Protocol.WhereSet.OP_OR));
//				whereList.add(new WhereSet("IDX_CAST_NAME_TW", Protocol.WhereSet.OP_HASALL, searchVO.getQ()));
				...
				//20170929 CAST_NAME 제외요청 반영
//				whereList.add(new WhereSet(Protocol.WhereSet.OP_OR));
//				whereList.add(new WhereSet("IDX_CAST_NAME_CHOSUNG_B", Protocol.WhereSet.OP_HASALL, searchVO.getQ()));
//				whereList.add(new WhereSet(Protocol.WhereSet.OP_OR));
//				whereList.add(new WhereSet("IDX_CAST_NAME_CHOSUNG_PR", Protocol.WhereSet.OP_HASALL, searchVO.getQ()));
...
//				whereList.add(new WhereSet(Protocol.WhereSet.OP_BRACE_CLOSE));
				...
				whereList.add(new WhereSet("IDX_BROADCASTER_F", Protocol.WhereSet.OP_HASALL, searchVO.getQ()));
				// whereList.add( new WhereSet(Protocol.WhereSet.OP_OR) );
				// whereList.add( new WhereSet("IDX_TITLE_ENG_B",
				// Protocol.WhereSet.OP_HASALL, searchVO.getQ()));
				whereList.add(new WhereSet(Protocol.WhereSet.OP_OR));
				whereList.add(new WhereSet("IDX_DIRECTOR_ENG_B", Protocol.WhereSet.OP_HASALL, searchVO.getQ()));
				whereList.add(new WhereSet(Protocol.WhereSet.OP_OR));
				whereList.add(new WhereSet("IDX_PLAYER_ENG_B", Protocol.WhereSet.OP_HASALL, searchVO.getQ()));
...
/*				 // 20160627LTE_MOV 제거
				...
				whereList.add(new WhereSet(Protocol.WhereSet.OP_OR));
				whereList.add(new WhereSet("IDX_ACTOR_TW", Protocol.WhereSet.OP_HASALL, searchVO.getQ().replaceAll("\\s", "")));
				 20160823 11시 : IDX_ACTOR_RB 추가-시작 
				whereList.add(new WhereSet(Protocol.WhereSet.OP_OR));
				whereList.add(new WhereSet("IDX_ACTOR_RB", Protocol.WhereSet.OP_HASALL, searchVO.getQ()));
				 20160823 11시 : IDX_ACTOR_RB 추가-끝 
...
//				whereList.add(new WhereSet(Protocol.WhereSet.OP_OR));
	 ...
	 * @return
	 */
	private WhereSet[] makeNewWhereSet(VideoVO searchVO) throws Exception {
...
//				whereList.add(new WhereSet("IDX_CAST_NAME_CHOSUNG_B", Protocol.WhereSet.OP_HASALL, searchVO.getQ()));
	 ...
	 * @return
	 */
	private OpenAPIVideoResponse makeResponse(VideoVO searchVO, Query[] queryList, Result[] resultList, OpenAPIVideoResponse response,
			String[] sectionListArr) throws Exception {
		Section_List sectionListVo = new Section_List();
		String typoResult = "";
		// String typoCorrectResult = "";
		String stcFlag = "";
		String synSearcher = ""; // 오타보정어
		...
		ArrayList xmlSectionList = new ArrayList(); // 순위정하기전 리스트
		ArrayList xmlSectionRealList = new ArrayList(); // 순위 정하고 담을 리스트

		List sectionName = new ArrayList();
		List sectionWeight = new ArrayList();
		List sectionFirstChan = new ArrayList();

		ArrayList xmlCategoryList = new ArrayList();

		for (int i = 0; i < resultList.length; i++) {
			Xml_Section xml_Section = new Xml_Section();
			...
			if (searchVO.getW().equals("IPTV_QUICK") && i == resultList.length - 1) {
				...
				if (i == 0) {
					Category_List catelist = new Category_List();
					catelist.setXml_category(getGroupResultMap(result));
					List<Category_List> list = new ArrayList<Category_List>();

					list.add(catelist);
					response.setCategory_list(list);
				}
				...
				if (i == 1 && resultList.length == 3) {
					...
					list = response.getCategory_list();

					if (result.getTotalSize() > 0) {
						List<Xml_Category> rList = list.get(0).getXml_category();
						Xml_Category voo = new Xml_Category();
						voo.setCat_name("LTE_CHA");
						voo.setTotcnt(String.valueOf(result.getTotalSize()));

						rList.add(voo);

						List<Xml_Category> resultList_after = new ArrayList<Xml_Category>();
						...
						for (int ck = 0; ck < num; ck++) {
							score[ck] = Integer.parseInt((String) rList.get(ck).getTotcnt());
						}
						...
						for (int p = 0; p < num; p++) {
							for (int q = 0; q < num; q++) {
								if (p == rank[q]) {
									resultList_after.add(rList.get(q));
								}
							}
						}

						catelist.setXml_category(resultList_after);
						list.clear();
						list.add(catelist);
						response.setCategory_list(list);
				...
				if (i == resultList.length - 1) {
					...
					// 참조용 파라미터 셋팅
					xml_Section.setSection_name(sectionListArr[i]); // 검색 섹션
					xml_Section.setUsrspec(""); // 검색 정보파일명
					xml_Section.setIdxname(""); // 검색 색인 파일명
					xml_Section.setQryflag(""); // 검색 질의어 요청 방식
					xml_Section.setSrtflag(""); //

					if (result.getRedirect() != null) {

						response.setDirecturl(String.valueOf(result.getRedirect())); // 바로가기
																						// url
					}
					...
					// 공통 파라미터 셋팅
					xml_Section.setTotcnt(String.valueOf(result.getTotalSize())); // 검색
																					// 결과
																					// 전체
																					// 건수
					totalSum = totalSum + result.getTotalSize();
					xml_Section.setMaxcnt(searchVO.getOutmax()); // 사용자 클릭 요청수
					xml_Section.setOutcnt(String.valueOf(result.getRealSize())); // xml
																					// 결과
																					// 출력
																					// 건수
					xml_Section.setPagenum(searchVO.getPg()); // xml 결과 출력 페이지
					String doc = ""; // id:문서번호, simc유사도
					String corqur = ""; // 오타보정 추천어

					ArrayList docList = makeVideoXmlDoc(searchVO, result, (Query) queryList[i], sectionListArr[i]);

					if (docList.size() > 0) {
						V_doc v_doc = (V_doc) docList.get(0);

						if (v_doc.getV_doc().size() > 0) {
							switch (sectionListArr[i]) {
							case "LTE_CHA": // 실시간TV
								sectionName.add(sectionListArr[i]);
								sectionWeight.add("2147483646"); // 실시간 티비가 검색된
																	// 것이 있으면
																	// 무조건 최상위
								sectionFirstChan.add("2147483646");
								break;
							default: // VOD
								sectionName.add(sectionListArr[i]);
								sectionWeight.add(v_doc.getV_doc().get(0).getV_att_vod().getWeight());
								sectionFirstChan.add("0");

								for (int vodnum = 0; vodnum < docList.size(); vodnum++) {

									V_doc v_doc_doc = (V_doc) docList.get(vodnum);

									V_vod vodvod = v_doc_doc.getV_doc().get(0).getV_att_vod();
									...
									v_doc_doc.getV_doc().get(0).setV_att_vod(vodvod);
									docList.set(vodnum, v_doc_doc);

								}

								// if (sectionListArr[i].equals("LTE_MOV")) {
								// for ( int vodnum
								// =0;vodnum<v_doc.getV_doc().size();vodnum++ )
								// {
								//
								// V_vod vodvod =
								// v_doc.getV_doc().get(vodnum).getV_att_vod();
								// String star=vodvod.getStarringActor(); // 주연
								// String support =vodvod.getSupportingActor();
								// // 조연
								// String extra=vodvod.getExtraActor(); // 단역
								// String actor=vodvod.getExtraActor(); // 출연
								//
								...
								// !support.equals("")) {
								// vodvod.setActorGb("조연");
								// }else if (extra!=null && !extra.equals("")) {
								// vodvod.setActorGb("단역");
								// }else if (actor!=null && !actor.equals("")) {
								// vodvod.setActorGb("출연");
								// } else {
								// vodvod.setActorGb("");
								// }
								// v_doc.getV_doc().get(vodnum).setV_att_vod(vodvod);
								//
								// }
								// }
					...
					xml_Section.setV_docs(docList);

					xmlSectionList.add(xml_Section);
				}
			}
		}
		...
		for (int ck = 0; ck < num; ck++) {
			score[ck] = Integer.parseInt((String) sectionWeight.get(ck));
		}
		...
		for (int p = 0; p < num; p++) {
			for (int q = 0; q < num; q++) {
				if (p == rank[q]) {
					xmlSectionRealList.add(xmlSectionList.get(q));
				}
			}
		}
		...
		if (response.getCorqry() != null && response.getCorqry().equals(searchVO.getQ())) {
		...
		// 검색 결과 담기
		sectionListVo.setXml_section(xmlSectionRealList);

		/*
		 * // 연관검색어 담기 response.setRelWord(relWordList); logger.debug(
		 * "[SEARCH RelWord CNT] "+relWordList.size());
		 */
		if (totalSum == 0) {
			response.setErrorResponse(UtilManager.makeRsInfo(-1));
		}
		List<Section_List> sectionList = new ArrayList<Section_List>();
		sectionList.add(sectionListVo);
		response.setSection_list(sectionList);
	 ...
	 * @return
	 */
	private OpenAPIVideoQuickResponse makeQuickResponse(VideoVO searchVO, Query[] queryList, Result[] resultList, OpenAPIVideoQuickResponse response,
			String[] sectionListArr) throws Exception {
		Section_List sectionListVo = new Section_List();
		String typoResult = "";
		// String typoCorrectResult = "";
		...
		ArrayList relWordList = new ArrayList();
		List wordList = new ArrayList();
		ArrayList xmlSectionList = new ArrayList();
		for (int i = 0; i < resultList.length; i++) {
			Xml_Section xml_Section = new Xml_Section();
			Result result = resultList[i];
			...
			if (searchVO.getW().equals("IPTV_QUICK") && i == resultList.length - 1) {
				...
				if (i == resultList.length - 1) {
					...
					// 공통 파라미터 셋팅
					xml_Section.setTotcnt(result.getTotalSize() + ""); // 검색 결과
																		// 전체 건수
					totalSum = totalSum + result.getTotalSize();
					xml_Section.setMaxcnt(searchVO.getOutmax()); // 사용자 클릭 요청수
					xml_Section.setOutcnt(String.valueOf(result.getRealSize())); // xml
		...
		// 검색 결과 담기
		sectionListVo.setXml_section(xmlSectionList);
	 ...
	 * @return
	 */
	private OpenAPIVideoResponseJson makeResponseJson(VideoVO searchVO, Query[] queryList, Result[] resultList, OpenAPIVideoResponseJson response,
			String[] sectionListArr) throws Exception {
		String typoResult = "";
		// String typoCorrectResult = "";
		String stcFlag = "";
		...
		for (int i = 0; i < resultList.length; i++) {
			...
			if (i == 0) {
				...
				stcFlag = result.getValue("stcflag");
				synSearcher = result.getValue("synsearCher");
				// response.setCorqry(result.getValue("synSearcher"));
				// relWordList = getDramaResult(result); // 연관검색어 결과
				// relWordSum = relWordSum+relWordList.size();
			}
			if (searchVO.getW().equals("IPTV_QUICK") && i == resultList.length - 1) {
		...
		// // 오타보정
		// if(synSearcher !=null && !"".equals(synSearcher) &&
		// !synSearcher.equals(searchVO.getQ())) {
		// response.setCorqry(synSearcher);
		//// } else if(typoCorrectResult != null &&
		// !typoCorrectResult.equals("")) {
		//// response.setCorqry(typoCorrectResult);
		// } else {
		// // typo의 영한 변환 결과가 이상할 경우 typo 결과를 사용하지 않음(정상적인 한글일 경우에만 typo 결과 사용)
		// // 한영 변환도 추가되었으므로 검색 쿼리가 이상할 경우 typo 결과 사용(영한 변환의 result를 확인하는 것과 동일한
		// 방법 사용)
		// // 생각을 바꿔서..
		// // 입력이 영어고 출력이 바른 한글일 경우에 처리
		// if(StringUtil.isEnglishHyphen(searchVO.getQ()) &&
		// StringUtil.isCorrecHangul(typoResult))
		// response.setCorqry(typoResult);
		// // 입력이 바른 한글이 아닌 경우에 대해서 처리
		// else if(!StringUtil.isCorrecHangul(searchVO.getQ()))
		// response.setCorqry(typoResult);
		// }
		//
		// if (response.getCorqry() != null &&
		// response.getCorqry().equals(searchVO.getQ()) ) {
		//
		// response.setCorqry(null);
		// }
	 ...
	 * @return
	 */
	private OpenAPIVideoQuickResponseJson makeQuickResponseJson(VideoVO searchVO, Query[] queryList, Result[] resultList,
	...
	private List makeAutoWord(Result result, Query query) throws Exception {
		...
		for (int i = 0; i < result.getRealSize(); i++) {
			...
			for (int k = 0; k < result.getNumField(); k++) {
				...
				// 수정후 2
				// if (selectFieldName.equals("KEYWORD") ) {
				// str += selectFieldValue;
				// }
				// else if ( selectFieldName.equals("POINT") ) {
				// str += " (POINT:"+selectFieldValue+ ")";
				// }
				// 수정후 2
			}
			// 수정후 3
	 ...
	 */
	private ArrayList makeVideoXmlDoc(VideoVO searchVO, Result result, Query query, String section) throws Exception {
		...
		ArrayList<V_doc> docList = new ArrayList<V_doc>();

		switch (collectionName) {
		case "I30_CHANNEL":
			List<V_att> channelList = new ArrayList<V_att>();
			for (int i = 0; i < result.getRealSize(); i++) {
				...
				V_att attCh = new V_att();
				attCh.setV_att_channel(v_channel);
				channelList.add(attCh);
			}
			V_doc docCh = new V_doc();
			docCh.setV_doc(channelList);
			docList.add(docCh);
			break;
		case "I30":
			List<V_att> vodList = new ArrayList<V_att>();
			for (int i = 0; i < result.getRealSize(); i++) {
				...
				for (int k = 0; k < result.getNumField(); k++) {
					...
					} else if (selectFieldName.equals("PT")) {
				...
				V_att attVod = new V_att();
				attVod.setV_att_vod(v_vod);
				vodList.add(attVod);
			}

			V_doc docVod = new V_doc();
			docVod.setV_doc(vodList);
			docList.add(docVod);
			break;
		default:
			break;
	 ...
	 * @return OpenAPIVideoResponseresult
	 */
	private List makeVideoJsonAtt(VideoVO searchVO, Result result, Query query, String section) throws Exception {
		String collectionName = "";
		switch (section) {
		case "LTE_CHA": // 대박영상
			collectionName = "I30_CHANNEL";
	...
	private OrderBySet[] makeOrderBySet(VideoVO searchVo) throws Exception {
		OrderBySet[] orderBySet = null;
		ArrayList orderByList = new ArrayList();
		String section = searchVo.getSection();

		if (searchVo.getCollection().equals("I30_AUTO_COMPLETE")) {
			orderByList.add(new OrderBySet(true, "SORT_POINT", Protocol.OrderBySet.OP_NONE)); // 포인트
																								// 내림차순

		} else {

			// if ( searchVo.getCollection().equals("IPTV_VOD")) {
			// orderByList.add(new OrderBySet(true, "SORT_CAT_NAME_CHAR",
			// Protocol.OrderBySet.OP_PREWEIGHT)); // 카테고리명
			// } else if ( searchVo.getCollection().equals("VIDEO_CHANNEL")) {
			// orderByList.add(new OrderBySet(true, "WEIGHT",
			// Protocol.OrderBySet.OP_NONE)); // 정확도 우선 내림차순
			// } else if ( searchVo.getCollection().equals("VIDEO_EVENT")) {
			// orderByList.add(new OrderBySet(true, "WEIGHT",
			// Protocol.OrderBySet.OP_NONE)); // 정확도 우선 내림차순
			// } else {
			// orderByList.add(new OrderBySet(true, "WEIGHT",
			...
			if (searchVo.getCollection().equals("I30")) {
				// orderByList.add(new OrderBySet(true, "SORT_CAT_NAME_CHAR",
				// Protocol.OrderBySet.OP_PREWEIGHT)); // 카테고리명
				// orderByList.add(new OrderBySet(true, "ORDER_DATE",
				// Protocol.OrderBySet.OP_NONE)); // 날짜 우선 내림차순
				...
				if (!"".equals(sort) && (sort != null)) {
					...
					// orderByList.add(new OrderBySet(true, "WEIGHT",
					// Protocol.OrderBySet.OP_NONE)); // 정확도
				}
	...
	private FilterSet[] makeFilterSet(VideoVO searchVo) throws Exception {
		...
		if (searchVo.getCollection().equals("I30_CHANNEL")) {
	...
	private GroupBySet[] getGroupBySet(VideoVO voSearch) {
	 ...
	 * @return
	 */
	private List<Xml_Category> getGroupResultMap(Result result) {

		ArrayList groupResultList = new ArrayList();
		List<Xml_Category> resultList = new ArrayList<Xml_Category>();
		...
		if (result != null && result.getGroupResultSize() != 0) {
			...
			for (int i = 0; i < 1; i++) {
				...
				for (int k = 0; k < groupResultlist[i].groupResultSize(); k++) {
					...
					Xml_Category vo = new Xml_Category();
