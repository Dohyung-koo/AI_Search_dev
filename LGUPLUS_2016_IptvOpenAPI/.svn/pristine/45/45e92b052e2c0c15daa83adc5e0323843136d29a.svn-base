...
public class StringUtil {
	...
	public static boolean isContain(Object object, String keyword, String gubun) {
		...
		if (isEmpty(object) && isEmpty(keyword))
			return false;
		StringTokenizer strToken = new StringTokenizer(String.valueOf(object),
				gubun);
	 ...
	 * @return boolean (검색어 포함여부)
	 */
	public static boolean isContainKeywords(Object object, String keywords,
			String gubun) {
	 ...
	 * @return String (변경후 문자)
	 */
	public static String insertion(Object object, String insertStr,
			int insertPoint, int limitLen) {
		if (object == null)
			return "";
		String str = String.valueOf(object);
		if (isBlank(str))
	 ...
	 */
	public static String replace(String originalStr, String oldStr,
			String newStr) {
	 ...
	 *            새로운 문자열.
	 * @param replaceCount
	 *            originalStr의 처음부터 몇개까지의 oldStr를 newStr로 바꿀지를 결정한다. 1이면 처음 나타나는
	 *            oldStr 문자열만을 newStr로 바꾼다. 0이면 모든 oldStr 문자열을 newStr로 바꾼다.
	 * @return originalStr 문자열에서 oldStr 문자열을 찾아 replaceCount 갯수만큼 newStr 문자열로
	 *         대치한 문자열을 넘긴다.
	 */
	public static String replace(String originalStr, String oldStr,
			String newStr, int replaceCount) {
		...
		while (true) {
			// System.out.println(String.format("oldstr=[%1$s], fromIndex[%2$s], src[%3$s]",oldStr,
			// fromIndex,
			// originalStr));
			...
			if (index < 0) {
				...
				result = result.substring(0, index) + newStr
						+ result.substring(index + oldLength);
			}
	...
	public static boolean isEnglishHyphen(String s) {
		boolean flg = true;
		char c = 'a';
		final int len = s.length();
		for (int i = 0; i < len; i++) {
	 ...
	 * @author hwado
	 */
	public static String Euc2Utf(String UnicodeStr)
			throws UnsupportedEncodingException {
		if (UnicodeStr == null) {
			return null;
		} else {
	...
	public static String ksc2Utf(String str)
			throws UnsupportedEncodingException {
		if (str == null) {
			return null;
		} else {
...
//	}
	...
	public static String SpaceJob(String field_value, String token, char gubun) {
		...
		case 'p':
			while (field_value.length() > cTempPos) {
				...
				if (cTempPos != -1) { // token이 존재하면
					if (field_value.substring(cTempPos - 1, cTempPos).equals(
							" ")) { // token 이전 문자가 공백이면
		...
		case 'n':
			while (field_value.length() > cTempPos) {
				cTempPos = field_value.indexOf(token, cTempPos);
				if (cTempPos != -1) { // token이 존재하면
					if (field_value.substring(cTempPos + 1, cTempPos + 2)
							.equals(" ")) { // token 이후 문자가 공백이면
						lfield_value = field_value.substring(0, cTempPos + 1);
			...
			while (true) {
				...
				if (cTempPos != -1) { // token이 존재하면
					if ((field_value.length() > cTempPos + 1)
							&& field_value
									.substring(cTempPos + 1, cTempPos + 2)
									.equals(" ")) { // token이후
													// 문자가
													// 공백이면
						lfield_value = field_value.substring(0, cTempPos + 1);
						...
						field_value = lfield_value + rfield_value;
							...
							&& (field_value.substring(cTempPos - 1, cTempPos)
									.equals(" "))) { // token이전
														// 문자가
														// 공백이면
						lfield_value = field_value.substring(0, cTempPos - 1);
	...
	public static String removeTag(String str) {
		...
		// 정규식 표현 미리 정의해놓음
		Pattern HTML_SCRIPT = Pattern.compile(
				"\\<script[^>]*?>.*?\\<\\/script\\>", Pattern.CASE_INSENSITIVE);
		Pattern HTML_STYLE = Pattern.compile(
				"\\<style[^>]*?>.*?\\<\\/style\\>", Pattern.CASE_INSENSITIVE);
		Pattern HTML_OPTION = Pattern.compile(
				"\\<option[^>]*?>.*?\\<\\/option\\>", Pattern.CASE_INSENSITIVE);
		Pattern HTML_HEAD = Pattern.compile("\\<head\\>(.*?)\\<\\/head\\>",
	 ...
	 * str.indexOf("<"); if ( lt != -1 ) { int gt = str.indexOf(">", lt); if (
	 * (gt != -1) ) { str = str.substring( 0, lt ) + str.substring( gt + 1 ); //
	 * 재귀 호출 str = removeTag(str); }else{ str = str.substring( 0, lt ); } }
	 * return str; }
	 */
	...
	public static String ToUpperOneChar(String s) {
		...
		while (st.hasMoreTokens()) {
			word = st.nextToken();
			writing += word.substring(0, 1).toUpperCase()
					+ word.substring(1, word.length()) + space;
		}
		return writing;
	}
	...
	public static String DateToDot2(String str, String dechar) {
		...
		case 7:
			return str.substring(0, 4) + dechar + "0" + str.substring(4, 5)
					+ dechar + str.substring(5, 7);
		case 8:
			return str.substring(0, 4) + dechar + str.substring(4, 6) + dechar
					+ str.substring(6, 8);
		}
		return str;
	}
	...
	/**
	 * removeWhitepaceAndSpChar 와 동일하지만,
	 * 연속적으로 나오는 공백은 1개로 축약하고, 마지막의 공백은 제거한다.
	 * @param s
...
//				newStr = val.substring(0, sp);
	...
	public static void format(StringBuilder trans, String[] source, String everyLeft, String everyRight, String left, String separator, String right) {
		trans.append(left);
		if (source.length > 0) {
	...
	public static boolean isCorrecHangul(String s) {
		
		boolean isJa = false;
		boolean isMo = false;
		// 자모가 연속해서 2개 나오면 이상한거다..
		// 초성검색이 있으므로 자음이 연속해서 나올 수는 있다.
...
//		String[] a = {"asdfeee", "asdfEee", "asdfEEE", "ASDFeee", "ASDFEEE"};
//		for (int i = 0; i < a.length; i++) {
//			String b = javaToDB(a[i]);
//			String c = toCamelCase(b);
////			String d = javaToDB(c);
////			System.out.println("a=" + a[i] + ", b=" + b + ", c=" + c + ", d=" + d);
//			System.out.println("a=" + a[i] + ", b=" + b + ", c=" + c);
//		}
		System.out.println(removeBigBrace("안녕하세요[hello](안녕하세요[hello])"));
		System.out.println(removeBrace("안녕하세요(hello)"));
		
		System.out.println(isEnglishHyphen("OHDaL SU"));
		
		System.out.println(isCorrecHangul("the lost"));
